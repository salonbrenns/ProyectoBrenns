...
export default function ProductDetailClient({ producto }: { producto: Producto }) {
  ...
  const incrementar = () => setCantidad((c) => Math.min(99, c + 1))
  ...
  const agregarAlCarrito = () => {
    try {
      ...
      toast.textContent = `¡${cantidad} x ${producto.nombre} agregado al carrito!`
      toast.style.cssText = `
        position:fixed;bottom:30px;left:50%;transform:translateX(-50%);
        background:#ec4899;color:white;padding:12px 28px;border-radius:999px;
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-center">
                <div className="inline-flex items-center gap-3 bg-white rounded-full p-1 shadow">
                  <button type="button" aria-label="Disminuir cantidad" onClick={decrementar} className="w-10 h-10 rounded-full flex items-center justify-center bg-white border border-pink-200 text-pink-600 hover:bg-pink-50">
                    <Minus className="w-4 h-4" />
                  </button>

                  <div className="px-6 text-lg font-bold">{cantidad}</div>

                  <button type="button" aria-label="Aumentar cantidad" onClick={incrementar} className="w-10 h-10 rounded-full flex items-center justify-center bg-white border border-pink-200 text-pink-600 hover:bg-pink-50">
                    <Plus className="w-4 h-4" />
                  </button>
                </div>
              </div>

              <button onClick={agregarAlCarrito} className="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold text-lg py-4 rounded-full shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95">
                <ShoppingBag className="w-5 h-5" /> Agregar {cantidad} al Carrito
              </button>
            </div>
