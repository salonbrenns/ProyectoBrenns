...
// src/components/servicios/ImageUploadServicio.tsx
import { useState, useRef, useCallback, useEffect } from 'react'
import Image from 'next/image'
import { X, Plus, Star, GripVertical } from 'lucide-react'

interface ImageUploadServicioProps {
...
export default function ImageUploadServicio({ initialImages = [] }: ImageUploadServicioProps) {
      ...
      <div className="grid grid-cols-2 gap-3">
        {slots.map((slot, i) => (
          <div
            ...
            className={`relative aspect-square rounded-xl overflow-hidden border-2 bg-pink-50 shadow-sm group cursor-grab active:cursor-grabbing transition-all duration-150 ${
              dragOver === i ? 'border-pink-500 scale-105 shadow-lg' : 'border-pink-100'
            }`}
          >
            <Image
              src={slot.url}
            ...
            />
            {i === 0 && (
              <span className="absolute top-1.5 left-1.5 z-10 flex items-center gap-0.5 bg-pink-700 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow">
            ...
            <button
              ...
              className="absolute top-1.5 right-1.5 z-10 bg-white/90 backdrop-blur-sm rounded-full p-1 shadow hover:text-pink-600 hover:scale-110 transition-all opacity-0 group-hover:opacity-100"
            >
        ...
        {slots.length < MAX && (
          <label className="aspect-square flex flex-col items-center justify-center border-2 border-dashed border-pink-200 rounded-xl hover:bg-pink-50 hover:border-pink-400 cursor-pointer transition-colors group">
            <Plus className="w-7 h-7 text-pink-300 group-hover:text-pink-500 transition-colors" />
