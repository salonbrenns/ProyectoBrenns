...
function ResetContenido() {
  ...
  useEffect(() => {
    ...
    if (!tokenParam || !emailParam) { setError("Enlace de reset inválido o expirado"); return }
    setToken(tokenParam)
    setEmail(emailParam)
  }, [searchParams])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    if (!allValidationsMet) { setError("La contraseña no cumple con todos los requisitos"); return }
    if (!passwordsMatch) { setError("Las contraseñas no coinciden"); return }
    if (!token || !email) { setError("Enlace inválido. Por favor solicita una nueva recuperación."); return }
    setLoading(true)
    try {
      ...
      if (!res.ok) throw new Error(data?.error || "Error al actualizar tu contraseña")
      setSuccess(true)
      ...
      <div className="min-h-screen flex flex-col md:flex-row font-sans">
        <div className="flex-1 bg-gradient-to-br from-pink-500 to-rose-600 p-8 md:p-12 flex flex-col justify-center items-start text-white relative overflow-hidden">
          <div className="z-10 relative">
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold mb-6 leading-tight">¡Contraseña <br /><span className="text-pink-200">Actualizada!</span></h1>
            <p className="text-lg md:text-xl text-pink-50 max-w-lg leading-relaxed">Serás redirigido al login en unos momentos.</p>
          </div>
        </div>
        <div className="flex-1 bg-pink-50 p-8 flex flex-col justify-center items-center">
          <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-xl border border-pink-100 text-center space-y-6">
            ...
            <Link href="/login" className="block w-full bg-pink-600 text-white py-3 rounded-full font-bold hover:bg-pink-700 transition-all">
              Ir a Iniciar Sesión
            </Link>
          </div>
        </div>
    ...
    <div className="min-h-screen flex flex-col md:flex-row font-sans">
      <div className="flex-1 bg-gradient-to-br from-pink-500 to-rose-600 p-8 md:p-12 flex flex-col justify-center items-start text-white relative overflow-hidden">
        <div className="z-10 relative">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold mb-6 leading-tight">Crea una Nueva <br /><span className="text-pink-200">Contraseña</span></h1>
          <p className="text-lg md:text-xl text-pink-50 max-w-lg leading-relaxed">Asegúrate de crear una contraseña fuerte que cumpla con todos los requisitos.</p>
        </div>
      </div>
      <div className="flex-1 bg-pink-50 p-8 flex flex-col justify-center items-center overflow-y-auto">
        <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-xl border border-pink-100">
          ...
          <p className="text-gray-500 text-center mb-8 text-sm">Correo: {email}</p>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              ...
              {[
                ...
                <div key={key} className={`flex items-center gap-2 text-xs ${validations[key as keyof typeof validations] ? "text-green-600" : "text-gray-500"}`}>
                  <div className={`w-3 h-3 rounded-full ${validations[key as keyof typeof validations] ? "bg-green-600" : "bg-gray-300"}`} />
                  {label}
                </div>
              ))}
            </div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••"
                  className={`w-full pl-12 px-4 py-2 rounded-lg border outline-none transition-all ${
                    confirmPassword === "" ? "border-gray-300 focus:border-pink-500"
            ...
            <label className="flex items-center gap-2 text-sm text-gray-600">
              <input type="checkbox" checked={showPassword} onChange={(e) => setShowPassword(e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-pink-600 cursor-pointer" />
              Mostrar contraseña
            </label>
            {error && <div className="p-3 rounded-lg bg-red-50 text-red-600 text-sm text-center border border-red-100">{error}</div>}
            <button type="submit" disabled={loading || !allValidationsMet || !passwordsMatch}
          ...
          <div className="mt-6 text-center">
            <Link href="/login" className="text-sm text-pink-600 font-bold hover:text-pink-700">Volver al login</Link>
          </div>
        </div>
      </div>
    </div>
