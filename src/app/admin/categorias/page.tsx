...
export default function CategoriasUnificadasPage() {
  ...
  const cargarProductos = useCallback(() => {
    setCargandoP(true)
    fetch(`/api/admin/categorias?query=${query}&page=${page}`)
      .then(r => r.json())
      .then(data => {
  ...
  const cargarServicios = useCallback(() => {
    setCargandoS(true)
    fetch(`/api/admin/categorias-servicios-lista?query=${query}&page=${page}`)
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <div className="space-y-6">
      ...
      <input
        type="text"
        defaultValue={query}
        key={tab} // resetea el input al cambiar pestaña
        placeholder={
          tab === 'productos'
